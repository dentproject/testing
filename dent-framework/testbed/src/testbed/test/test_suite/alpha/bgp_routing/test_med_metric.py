...
from testbed.utils.test_utils.bgp_routing_utils import (
...
@pytest.mark.asyncio
async def test_alpha_lab_bgp_routing_med_metric(testbed):
    """
    Test Name: test_alpha_lab_bgp_routing_med_metric
    Test Suite: suite_bgp_routing
    Test Overview: test BGP routing med metric
    Test Procedure:
    1. Test support for MEDs
    2. 2 routes learned from the WAN,
    3.  Assign a MED on prefix learned on bf_r1 using route-map and prefix-list do not do so on bf_r2
    4. Validate that that prefix is learned on RSW with the same the different MED (as opposed to the same from a different source)
    """
    ...
    for dd in devices[:1]:
        for i in range(num_routes):
            ...
            for path in route_info["paths"]:
                if "metric" not in path or path["metric"] != test_metric:
                    assert (
                        0
                    ), f"30.0.{i}.0/24 does not have a med set {dd.host_name} to {test_metric}"
