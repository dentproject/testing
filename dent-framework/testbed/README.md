...
```
 ...
 - Package or module level fixtures can be defined in respective folders as shown above. But need to be registered in conftest.py as shown below.
   `pytest_plugins = ["testbed.test.connection.fixtures"]`
...
- test suite and mapping from test suite to test suite group needs to be defined in `src/testbed/constants.py`
...
- For tests that are part of test framework needs to be packaged with the build. So they should be part of "src/testbed/test" folder.
