...
class ReportPyObject(object):
    ...
    def generate_code(self):
        ...
        self._imports.append(PyImport("(LeafSchemaDict, SchemaList, SchemaDict, Report)", _from="testbed.discovery.Report "))
...
class DiscoveryModulePyObject(object):
    ...
    def generate_code(self):
        self._imports.append(PyImport("Module",
                                      _from="testbed.discovery.Module "))
...
class DiscoveryPlugin(SamplePlugin):
    ...
    def generate_code(self, dbs, odir):
        ...
        tdir = os.path.join(odir, "src/testbed/discovery/")
        ...
        tdir = os.path.join(odir, "src/testbed/discovery/modules")
        if not os.path.exists(tdir):
            os.makedirs(tdir)
            fname = os.path.join(tdir, "__init__.py")
            f = open(fname, "w")
