...
in platform command model
   src/testbed/<module>/<dent_api_class>.yaml
   src/testbed/<module>/<platform>/<platform_api_class>.yaml
   src/testbed/<module>/<platform>/<pltform_api_class>_impl.yaml

  The test writers would use testbed.<module>.<dent_api_class>.NetProdClass.<api>
...
class TestPyObject(object):
    ...
    def generate_code(self):
        ...
        self._imports.append(PyImport("TestLibObject", _from="testbed.lib.test_lib_object "))
        for impl in self._cls.implemented_by:
            ...
            self._imports.append(
                PyImport(
                    "%(pd_impl_cc)s " % args,
                    _from="testbed.lib.%(cls_mod_name)s.%(pd_impl)s."
                    % (args)
                    + impl.name
                    + "_impl ",
                )
...
class TestPlugin(SamplePlugin):
    ...
    def generate_code(self, dbs, odir):
        ...
        tdir = os.path.join(odir, "src/testbed/lib/")
