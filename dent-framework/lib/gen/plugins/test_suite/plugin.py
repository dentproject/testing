...
class TestSuitePyObject(object):
    ...
    def generate_code(self):
        ...
        self._imports.append(PyImport("TestCaseSetup", _from="testbed.utils.decorators "))
        imported = {}
        classes = ''
            ...
            if tc.template not in imported:
                self._imports.append(
                    PyImport(
                        ...
                        _from="testbed.test.lib.%s " %(tc.template)
                    )
                )
...
class TestSuitePlugin(SamplePlugin):
    ...
    def generate_code(self, dbs, odir):
        ...
        tdir = os.path.join(odir, "src/testbed/test/test_suite/generated")
