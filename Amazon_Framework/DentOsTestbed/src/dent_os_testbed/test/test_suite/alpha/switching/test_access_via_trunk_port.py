...
async def test_alpha_lab_switching_vlan_access_via_trunk_port(testbed):
    # Validate device on an access port can communicate through an upstream trunk.
    # configure an access port on switch 1 connect host1 to it
    # configure a trunk port between switch1 and switch2
    # configure an access port on switch2 connect host2 on it
    # Validate host1 and host2 can communicate
