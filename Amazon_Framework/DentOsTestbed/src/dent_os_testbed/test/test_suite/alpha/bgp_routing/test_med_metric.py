...
async def test_alpha_lab_bgp_routing_med_metric(testbed):
    # Test support for MEDs
    # 2 routes learned from the WAN,
    #  Assign a MED on prefix learned on bf_r1 using route-map and prefix-list do not do so on bf_r2
    # Validate that that prefix is learned on RSW with the same the different MED (as opposed to the same from a different source)
    ...
    cmds.extend(
        [
            f"vtysh -c 'conf terminal' -c 'route-map FROM-IXIA permit 10' -c 'match ip address prefix-list IXIA-ROUTES' -c 'set metric {test_metric}'",
    ...
    for dd in devices[:1]:
        for i in range(num_routes):
            ...
            for path in route_info["paths"]:
                if "med" not in path or path["med"] != test_metric:
                    assert (
                        0
