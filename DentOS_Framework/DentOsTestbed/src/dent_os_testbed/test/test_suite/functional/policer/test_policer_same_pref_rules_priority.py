...
async def test_policer_same_pref_rules(testbed, qdisc_type):
    """
    Test Name: Policer rules priority
    Test Suite: suite_functional_policer
    Test Overview: Verify prioritization between 2 rules with the same priority (last one should be preferred )
    Test Procedure:
    ...
    4. Create within the ingress qdisc two rules with:
        # - same selectors,
        # - different police rate value,
        # - second rule pref = first rule pref
        # - second rule handle != first rule handle
    ...
    9. Verify traffic is handled by the first rule added to the qdisc
    """
    ...
    tc_rule_1_frame_rate = 250_000  # bps
    tc_rule_2_frame_rate = 500_000  # bps
    tolerance = 0.12
    ...
    rules = [{
        ...
        },
        {
            'action': {
                'police': {
                    'rate': tc_rule_2_frame_rate,
                    'burst': tc_rule_2_frame_rate + 1000,
                    'conform-exceed': 'drop'}
            },
            'direction': 'ingress',
            'protocol': '0x8100 ',
    ...
    for rule in rules:
        rule.update(dev_or_block)

    out = await TcFilter.add(input_data=[{dent: rules}])
    ...
    tc_filter_to_delete.update(dev_or_block)
    out = await TcFilter.delete(input_data=[{dent: [tc_filter_to_delete]}])
    assert out[0][dent]['rc'] == 0, 'Failed to delete tc rule '

    out = await TcFilter.add(input_data=[{dent: [rules[0]]}])
    assert out[0][dent]['rc'] == 0, 'Failed to create tc rule'
