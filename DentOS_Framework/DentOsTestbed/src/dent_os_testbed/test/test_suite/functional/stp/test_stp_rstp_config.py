...
async def test_stp_hello_time(testbed, version):
    """
    Test Name: test_stp_hello_time
    Test Suite: suite_functional_stp
    Test Overview: Verify the root bridge sends hello messages within the interval set
    Test Procedure:
    1. Add bridge
    2. Enslave ports
    3. Enable STP/RSTP
    4. Start capturing packets for 30 seconds
    5. Verify around 15 BPDUs received
    6. Change the hello time to 1
    7. Start capturing packets for 30 seconds
    8. Verify around 30 BPDUs received
    """
    ...
    # 4. Start capturing packets for 30 seconds
    captured = await tb_device_tcpdump(dent_devices[0], port, '-n llc', count_only=True, timeout=timeout_s)

    # 5. Verify around 15 BPDUs received
    assert is_close(int(captured), timeout_s // hello_time_s, rel_tol=tolerance), \
        f'Expected to capture {timeout_s // hello_time_s} BPDUs, but only got {captured}'

    # 6. Change the hello time to 1
    hello_time_s = 1
    out = await Mstpctl.set(input_data=[{dent: [
        {'parameter': 'hello', 'bridge': bridge, 'hello_time': hello_time_s}
    ]}])
    assert out[0][dent]['rc'] == 0, 'Failed to set hello time'
