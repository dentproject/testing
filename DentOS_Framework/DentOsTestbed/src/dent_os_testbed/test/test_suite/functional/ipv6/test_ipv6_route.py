...
pytestmark = [
    ...
    pytest.mark.usefixtures('cleanup_ip_addrs', 'cleanup_tgen', 'enable_ipv6_forwarding'),
    pytest.mark.asyncio,
]
...
async def test_ipv6_icmp(testbed):
    ...
    streams = {
        'icmpv6 unknown ip': {
            ...
            'dstMac': dut_mac[ports[0]],
            'srcIp': dev_groups[tg_ports[0]][0]['ip'],
            'dstIp': address_map[1].swp_ip[:-1] + '5',  # 2001:2222::5
            'ipproto': 'icmpv6',
            'icmpType': '128',
            'icmpCode': '0',
            'rate': 3,  # pps
    ...
    await asyncio.sleep(5)  # make sure that traffic is definitely stopped
    await tgen_utils_clear_traffic_items(tgen_dev)
    await tgen_utils_setup_streams(tgen_dev, None, streams)

    tcpdump = asyncio.create_task(tb_device_tcpdump(dent_dev, address_map[0].swp,
                                                    '-n -c 10 "icmp6 && ip6[40] == 1 && ip6[41] == 3"',
                                                    count_only=True, timeout=timeout))
    ...
    # Verify ICMP 'host unreachable' message is sent
    captured = await tcpdump
    assert captured > 0, 'Expected DUT to send ICMPv6 "host unreachable" reply'
    ...
    streams = {
        'icmpv6 hop limit 1': {
            ...
            'dstMac': dut_mac[ports[0]],
            'srcIp': dev_groups[tg_ports[0]][0]['ip'],
            'dstIp': dev_groups[tg_ports[1]][0]['ip'],
            'ipproto': 'icmpv6',
            'icmpType': '128',
            'icmpCode': '0',
            'rate': 3,  # pps
    ...
    tcpdump = asyncio.create_task(tb_device_tcpdump(dent_dev, address_map[0].swp,
                                                    '-n -c 10 "icmp6 && ip6[40] == 3 && ip6[41] == 0"',
                                                    count_only=True, timeout=timeout))
    ...
    # Verify ICMP 'time exceeded' message is sent
    captured = await tcpdump
    assert captured > 0, 'Expected DUT to send ICMPv6 "time exceeded" reply'
    ...
    address_map = (
        addr_info(ports[0], tg_ports[0], '2001:1111::1', '2001:1111::2', 64),
        addr_info(ports[1], tg_ports[1], '2001:1111::f', '2001:1111::1f', 64),
    )
    out = await IpAddress.add(input_data=[{dent: [
        {'dev': info.swp, 'prefix': f'{info.swp_ip}/{info.plen}'}
        for info in address_map
    ...
    # 6. Send ICMP echo request from TG
    streams = {
        f'icmpv6 {tg_ports[0]} -> {tg_ports[1]}': {
            'type': 'raw',
            'protocol': 'ipv6',
            'ip_source': dev_groups[tg_ports[0]][0]['name'],
            'ip_destination': dev_groups[tg_ports[1]][0]['name'],
    ...
    # Verify ICMP message redirect is sent
    captured = await tcpdump
    assert captured > 0, 'Expected DUT to send ICMPv6 "redirect" reply'
