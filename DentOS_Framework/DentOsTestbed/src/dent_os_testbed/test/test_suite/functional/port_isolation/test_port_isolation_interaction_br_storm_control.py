...
async def test_port_isolation_interaction_br_storm_control(testbed):
    """
    Test Name: test_port_isolation_interaction_br_storm_control
    ...
    6.  Set up the following streams with random generated size of packet:
            - broadcast stream on the first (isolated) TG port
            - broadcast stream on the fourth (non-isolated) port
    7.  Set storm control rate limit of broadcast streams.
    ...
    await devlink_rate_value(dev=f'pci/0000:01:00.0/{ports[0].replace("swp","")}',
                             name='bc_kbyte_per_sec_rate ', value=kbyte_value[0],
