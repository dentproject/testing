...
async def test_storm_control_unknown_unicast_traffic(testbed):
    """
    Test Name: test_storm_control_unknown_unicast_traffic
    Test Suite: suite_functional_storm_control
    Test Overview: Verify Unknown Unicast traffic is limited on the RX port by Storm Control.
    ...
    5.  Verify that storm control is disabled by default for unknown unicast traffic
        by setting the storm control rate limit to unknown unicast traffic on the TX port.
    6.  Set up the following stream:
        - unknown unicast traffic stream with random generated packet size on TX port.
    7.  Transmit continues traffic by TG.
    8.  Verify the RX rate on the RX port is as expected - the rate is limited by storm control.
    """
    ...
    try:
        ...
        streams = {
            'stream_A': {
                ...
                'srcMac': 'b2:f1:46:1d:97:ba',
                'dstMac': '3e:f4:7a:e8:10:34',
