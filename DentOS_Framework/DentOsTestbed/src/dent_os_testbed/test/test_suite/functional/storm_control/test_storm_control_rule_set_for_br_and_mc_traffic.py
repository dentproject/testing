...
async def test_storm_control_rule_set_for_br_and_mc_traffic(testbed):
    ...
    try:
        ...
        await devlink_rate_value(dev=f'pci/0000:01:00.0/{ports[0].replace("swp","")}',
                                 name='unreg_mc_kbyte_per_sec_rate', value=kbyte_value_mc,
                                 cmode='runtime', device_host_name=device_host_name, set=True, verify=True)
        await asyncio.sleep(traffic_duration)
        ...
        collected = {row['Traffic Item']:
                     {'tx_rate': row['Tx Rate (Bps)'], 'rx_rate': row['Rx Rate (Bps)']} for row in stats.Rows}
        for x in range(2):
            assert math.isclose(float(collected[f'stream_{1 if x==0 else 3}']['tx_rate']),
                                float(collected[f'stream_{1 if x==0 else 3}']['rx_rate']), rel_tol=deviation), \
                f"The rate is limited by storm control, actual rate \
                    {float(collected[f'stream_{1 if x==0 else 3}']['tx_rate'])} \
                        instead of {float(collected[f'stream_{1 if x==0 else 3}']['rx_rate'])}."
        assert math.isclose(kbyte_value_mc*1000, float(collected['stream_2']['rx_rate']), rel_tol=deviation), \
            f"The rate is not limited by storm control, \
                        actual rate {kbyte_value_mc*1000} instead of {float(collected['stream_2']['rx_rate'])}."
    finally:
