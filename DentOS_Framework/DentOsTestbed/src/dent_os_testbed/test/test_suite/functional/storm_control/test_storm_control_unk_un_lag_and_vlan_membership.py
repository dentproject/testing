...
async def test_storm_control_unk_un_lag_and_vlan_membership(testbed):
    """
    Test Name: test_storm_control_unk_un_lag_and_vlan_membership
    ...
    9.  Set up the following streams:
        - Ixia port 1: unknown unicast streams with random generated size of packet.
    ...
    params = [
        {'port': ports[0], 'name': 'unk_uc_kbyte_per_sec_rate', 'value': kbyte_value},
    ...
    try:
        ...
        streams = {
            f'stream_{x+1}_unk_un_swp1->swp{x+2}': {
                'ip_source': dev_groups[tg_ports[0]][0]['name'],
                'ip_destination': dev_groups[tg_ports[x+1]][0]['name'],
                'srcMac': f'f8:d4:6b:50:c9:f{x+7}',
                'dstMac': f'b6:ee:e2:ef:d7:6{x+6}',
        ...
        await verify_expected_rx_rate(kbyte_value, stats,
                                      rx_ports=[streams['stream_1_unk_un_swp1->swp2']['ip_destination']],
                                      deviation=0.10)
    finally:
