...
async def test_bridging(testbed):
    ...
    out = await IpLink.set(
        input_data=[{dut.host_name: [{"device": "swp1", "mtu": 9500}]}],
        input_data=[{dut.host_name: [{"device": "swp2", "mtu": 9500}]}],
        input_data=[{dut.host_name: [{"device": "swp3", "mtu": 9500}]}],
        input_data=[{dut.host_name: [{"device": "swp4", "mtu": 9500}]}])
    assert out[0][dut.host_name]["rc"] == 0, out
