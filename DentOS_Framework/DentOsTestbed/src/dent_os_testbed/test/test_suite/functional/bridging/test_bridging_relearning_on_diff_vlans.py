...
async def test_bridging(testbed):
    """
    Test Name: test_bridging_relearning_on_diff_vlans
    ...
    out = await BridgeVlan(
        input_data=[{dut.host_name: [{"device": "swp1", "vid": 3}]}],
        input_data=[{dut.host_name: [{"device": "swp2", "vid": 3}]}],
        input_data=[{dut.host_name: [{"device": "swp3", "vid": 3}]}])
    assert out[0][dut.host_name]["rc"] == 0, out
